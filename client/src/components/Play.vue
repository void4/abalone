...
export default {
  ...
  methods: {
    ...
    getGame() {
      const path = `${window.location.protocol}//${window.location.hostname}/abasocket/game`;
  ...
  created() {
    ...
    this.source = new EventSource(`${window.location.protocol}//${window.location.hostname}/abasocket/stream?token=${token}&tmpid=${tmpid}`);
